...
/**
 * Handler for adding error logging to all catch blocks in the current method.
 * Keyboard shortcut: Ctrl+Alt+R
 */
public class LogErrorsMethodHandler extends BaseLogHandler {
    ...
    protected void executeOperation(OperationContext context, String selectedText) throws Exception {
        ...
        for (CatchClause catchClause : catches) {
            ...
            if (catchBody != null) {
                // Check if there's already a log statement
        ...
        if (count > 0) {
            ...
            logSuccess("Added error logging to " + count + " catch blocks in method '"
                    + ASTUtil.getName(method) + "'");
        } else {
            logWarning("No catch blocks without logging found in method");
        }
    }
}
